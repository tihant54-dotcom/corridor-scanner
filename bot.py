    ...
    def _load(self):
        if os.path.exists("state.json"):
            try:
                ...
                self.demo = True if config.FORCE_DEMO else d.get("demo", True)
            except Exception:
                pass
...
async def api_scan(request):
    """API endpoint для Mini App — возвращает реальные коридоры"""
    try:
        scanner = CorridorScanner(demo_mode=False)
        corridors = await scanner.scan_all()
        result = [c.to_dict() for c in corridors[:20]]
...
async def start_web_server():
    ...
    await site.start()
    logger.info("Web API запущен на порту 8080")
